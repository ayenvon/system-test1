...
from controller.stripe_auth import stripe_auth_checker
...
async def run_checks(cards, sites, proxy, max_concurrent: int = 500):
    ...
    async def wrapped_checker(card, site):
        async with sem:
            try:
                await asyncio.shield(stripe_auth_checker(card, site, proxy))
